...
export const useTaskStore = defineStore("tasks", {
    ...
    actions: {
        async fetchTasks() {
            //preguntar alex si esto está bien, ya que no quiero cebar a peticiones por cada ruta
            ...
            if (tasks) {
                ...
                let archievedTasks = tasks.filter(
                    (task) => task.is_archieved == true
                );
                let restTasks = tasks.filter(
                    (task) =>
                        task.is_archieved == false && task.is_complete == false
                );
