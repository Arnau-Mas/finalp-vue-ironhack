<template>
    <div :class="classesString">
        ...
        <button
            ...
            v-tooltip="'Unarchive'"
            class="flex-shrink-0 border-transparent border-4 text-gray-500 hover:text-green-800 text-sm rounded"
            type="button"
        >
            <svg
        ...
        <button
            @click="isDeleted(taskIdProp)"
            v-tooltip="'Delete'"
            class="flex-shrink-0 border-transparent border-4 text-red-400 hover:text-red-500 text-sm py-1 ml-2 mr-2 rounded"
            type="button"
        >
            <svg
                xmlns="http://www.w3.org/2000/svg"
...
<script setup>
    ...
    async function isDeleted(idTask) {
        ...
        try {
            ...
            if (res) {
                let indexTask = tasks.tasksCompleted.findIndex(
                    (task) => task.id == idTask
                );
                setTimeout(() => {
