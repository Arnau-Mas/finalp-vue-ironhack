            ...
            <div class="bg-white rounded-lg p-10 max-w-md w-full space-y-8">
                <div>
                    ...
                    <p class="mt-2 text-center text-sm text-gray-600">
                        The best<span class="font-medium text-cyan-600"> To-do app</span> in the world!
                    </p>
                </div>
                <form
                ...
                >
                    ...
                    <div class="rounded-md shadow-sm">
                        <div class="mb-4">
                            ...
                            <input
                                v-model="emailData"
                                class="appearance-none rounded relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-cyan-500 focus:border-cyan-500 focus:z-10 sm:text-sm"
                                type="email"
                                name="email"
                                id="email"
                                required
                    ...
                    <div>
                        <button
                            class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-0 focus:ring-offset-0 focus:ring-cyan-500"
                        >
                            Sign up
                        </button>
                    </div>
...
<script setup>
  ...
  async function submitData() {
      ...
      if (response.value === 1) {
          try {
              ...
              alertMessage.value =
                  "Pleas, check your email to activate your account.";
  ...
  function checkAllData() {
      ...
      if (!emailData.value || !password1Data.value || !password2Data.value) {
          printErr("errorMsg", "Pleas, introduce all the data.", true),
              (res.value = -1);
      } else {
          let checkEmail = validateEmail();
          if (checkEmail == -1) {
              printErr("errorMsg", "Pleas, introduce valid email.", true),
                  (res.value = -1);
          } else {
