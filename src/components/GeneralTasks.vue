<template>
    <div class="ml-2 mr-2 flex flex-col items-center mt-10 sm:mt-20 pb-16">
        <NewTask />
        <section
            v-if="allTasks.length && noError"
            class="flex flex-col items-center w-full"
        >
            <TaskItem
                v-for="task in allTasks"
                :key="task.id"
                :taskTextProp="task.title"
                :taskTimeProp="task.time"
                :taskIdProp="task.id"
            />
        </section>
        <section v-else class="flex flex-col items-center w-full mt-2">
            <p v-if="!noError">
                There has been some error. Pleas, reload or try it later.
            </p>
            <AlertMessage
                v-else
                :messageClass="'writeYourTask'"
                :alertMessage="'There are no tasks. Write up here!'"
                ><svg
...
<script setup>
    ...
    async function getTasks() {
        if (tasks.tasks == null) {
            try {
                //preguntar alex si esta logica es correcta
